...
export class MatriculaAtividadeService {
  ...
  async create(createDto: CreateMatriculaAtividadeDto) {
    const existingMatricula = await this.prisma.matriculaAtividade.findUnique({
      where: {
    ...
    if (existingMatricula) {
      throw new ConflictException("Esta pessoa já está matriculada nesta atividade.");
    }

    const id = this.snowflakeService.generate();
    const data: Prisma.MatriculaAtividadeCreateInput = {
      id_matricula: id,
      pessoa: { connect: { id_pessoa: BigInt(createDto.id_pessoa) } },
